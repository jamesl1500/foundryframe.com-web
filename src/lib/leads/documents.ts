...
export type ProposalDocumentFormat = "docx" | "pdf";
...
export async function renderPdf(
  ...
  for (const block of getBlocks(proposal, kind)) {
    if (block.style === "spacer") {
      y -= 16;
      continue;
    }

    const isStrong = block.style === "title" || block.style === "subtitle" || block.style === "heading" || block.style === "label";
    const font = isStrong ? bold : regular;
    const fontSize = block.style === "title" ? 25 : block.style === "heading" ? 16 : block.style === "subtitle" ? 12 : block.style === "label" ? 8 : 10;
    const color = block.style === "label" ? rgb(0.4, 0.4, 0.4) : rgb(0.08, 0.08, 0.08);
    const prefix = block.style === "bullet" ? "- " : "";
    const lines = wrapText(`${prefix}${block.text}`, font, fontSize, pageSize[0] - margin * 2);
    const lineHeight = fontSize * 1.45;
    const topSpacing = block.style === "heading" ? 14 : block.style === "title" ? 8 : 3;
    const neededHeight = topSpacing + lines.length * lineHeight + 5;
  ...
  pages.forEach((currentPage, index) => {
    currentPage.drawText(`Foundry Frame | ${index + 1} of ${pages.length}`, {
      x: margin,
      y: 24,
      size: 8,
      font: regular,
  ...
  pdf.setTitle(kind === "proposal" ? proposal.title : `${proposal.prepared_for} Services Agreement`);
  pdf.setAuthor("Foundry Frame LLC");
  pdf.setSubject("Draft generated by the Foundry Frame proposal workbench");
  return pdf.save();
}